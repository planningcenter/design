...
<button
  ...
  onClick={() => {
    Swal.fire(
      "This is an old SweetAlert",
      "The goal here is to ensure that we didn't clobber existing alerts. We want to make sure those con work in tandem with new alerts to allow a staged migration.",
      "warning"
    );
  }}
>
  Trigger warning likely to be seen in legacy code
</button>
